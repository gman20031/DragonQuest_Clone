...
namespace BlackBoxEngine
{
    ...
    class OpenSimplex : public NoiseGenerator
    {
        OpenSimplexNoise::Noise* m_pGenerator;
    public:
        OpenSimplex( uint32_t seed, const char* pName );
        virtual ~OpenSimplex();
        ...
        virtual void Reseed( uint32_t seed ) override;
        virtual float Noise2d( float x, float y, uint32_t seedOverride ) const override;
        float Noise3d( float x, float y, float z ) const;
    private:
        float NoiseWorld3d(
            float x, float y,
            float z, float worldWidth,
            float worldHeight, float noiseInputRange ) const;
    };
}